//
//  moreInforTableViewController.m
//  tabswashcars
//
...
@interface moreInforTableViewController (){
...
@implementation moreInforTableViewController
...
{
    //根据页面类型不同需要配置的部分
    _iPs_PageName=@"用户评论"; //页面名称(用于标记页面参数配置)
    ...
    _iPs_POSTAction =@"goods_new_comment"; //用户评论
...
- (void)addFooter
{
    __unsafe_unretained moreInforTableViewController *vc = self;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //使用自定义的单元格
    commitViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier  forIndexPath:indexPath];
    ...
    @try {
        ...
        cell.lbUsername.text = [dict objectForKey:@"username"];
        cell.lbContent.text =[dict objectForKey:@"content"];
        
        double ldRank = [lsRank doubleValue];
        cell.ratingLabel.text = lsRank;
        
        [cell.starView setImagesDeselected:@"Star0.png" partlySelected:@"Star1.png" fullSelected:@"Star2.png" andDelegate:cell];

        [cell.starView displayRating:ldRank];
